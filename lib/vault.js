...
module.exports = {
    ...
    async init(web3, miningLogger) {
        ...
        if (requirePassword) {
            await this.askUserPassword(false).then((userPassword) => {
                cryptr = new Cryptr(userPassword);
            },(err) => {
                throw new Error(err);
            }).catch((err) => {
                miningLogger.appendToErrorLog(err);
        ...
        if (!this.encryptedStorageExists()) {
            ...
            mkdirp(this.get0xBitcoinLocalFolderPath(), (err) => {
                if (err) console.error(err)
                else console.log("\x1b[38;5;249m[" + new Date().getTime()+ //toString("[yy-MM-dd HH:mm:ss.SSS]") +
								 "]\x1b[0m created new keystore file at" + this.get0xBitcoinLocalFolderPath())
            });
        }
        ...
        if (!this.vaultDataExists(localStorage)) {
            localStorage.setItem('vaultdata',
                                 JSON.stringify({ vault: { data: this.getInitialVaultDataJson(),
...
//            prompt.start();
            ...
            prompt.get(schema, (err, result) => {
                ...
                return fulfilled(result.password);
            });
        });
    },
    ...
    async handleAccountCommand(subsystem_command, subsystem_option) {
        if (subsystem_command === 'new') {
            ...
            var unlocked = await this.init(web3, miningLogger);
        ...
        if (subsystem_command === 'export') {
            console.log('Ethereum accounts list:');
            this.printAccountsList(true)
    ...
    async getEtherBalance(address) {
        var balanceWei = await this.web3.eth.getBalance(address, "latest");
    ...
    selectAccount(public_address) {
        if (web3Utils.isAddress(public_address)) {
            if (this.hasAddress(public_address)) {
                ...
                //console.log('\n')
                miningLogger.print('Selected account:', public_address)
            } else {
                //console.log('\n')
                ...
                //console.log('\n')
                miningLogger.print('Selected account (READ ONLY):', public_address)
            }
        } else {
            //console.log('\n')
    ...
    setCPUThreads(threadcount) {
        ...
        if (int_threads >= 1 && int_threads < 6400) {
            ...
            miningLogger.print('Set thread count: ', int_threads);
        } else {
            console.log('You really have', int_threads, "CPU cores??? ... try again.");
        }
    },
    ...
    setGasPrice(gprice) // in gwei
    {
        ...
        if (f_gprice > 0.0001 && f_gprice < 100000) {
            ...
            miningLogger.print('Set gas price (gwei):', f_gprice);
        }
    ...
    setWeb3ProviderUrl(url) {
        ...
        miningLogger.print('Set web3 provider url:', url);
    },
    ...
    selectContract(contract_address) {
        if (web3Utils.isAddress(contract_address)) {
            ...
            miningLogger.print('Selected contract to mine:', contract_address)
        } else {
    ...
    selectMiningPool(pool_url) {
        ...
        miningLogger.print('Selected mining pool:', pool_url)
    },
    ...
    printAccountsList(withPrivateKey) {
        list = vaultData.account_list;
        ...
        for (var i in list) {
            ...
            if (withPrivateKey && acct.hasOwnProperty('privateKey') && acct.privateKey != null) {
    ...
    getDefaultContractAddress() {
        return ContractInterface.networks.mainnet.contracts._eticarelease.blockchain_address
    },
    ...
    async saveVaultData(vData) {
        ...
        for (var i in vData.account_list) {
            if (vData.account_list[i].hasOwnProperty('privateKey') && vData.account_list[i].privateKey != null) {
                await this.assertValidPassword()
                ...
                delete vData.account_list[i].privateKey;
            }
        }
